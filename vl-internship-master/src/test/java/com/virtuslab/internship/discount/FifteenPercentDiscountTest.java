...
class FifteenPercentDiscountTest {
    ...
    @Test
    void shouldNotApplyDiscountWhenAnyProductIsGrain() throws Exception {
        var productDb = new ProductDb();
        ...
        var discount = new FifteenPercentDiscount();
        var receiptAfterDiscount = discount.apply(receipt);
        var expectedTotalPrice = butter.price().add(cheese.price().multiply(BigDecimal.valueOf(2)));
    ...
    @Test
    void shouldNotApplyDiscountWhenThereIsOnlyOneGrainProduct() throws Exception {
        var productDb = new ProductDb();
        var cheese = productDb.getProduct("Cheese");
        var bread = productDb.getProduct("Bread");
        List<ReceiptEntry> receiptEntries = new ArrayList<>();
    ...
    @Test
    void shouldNotApplyDiscountWhenThereAreOnlyTwoGrainProduct() throws Exception {
        var productDb = new ProductDb();
        var cheese = productDb.getProduct("Cheese");
        var bread = productDb.getProduct("Bread");
        var cereals = productDb.getProduct("Cereals");
        ...
        var discount = new FifteenPercentDiscount();
        var receiptAfterDiscount = discount.apply(receipt);
        var expectedTotalPrice = (cheese.price().multiply(BigDecimal.valueOf(2))).add(bread.price()).add(cereals.price());
